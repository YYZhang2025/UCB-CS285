...
class PGAgent(nn.Module):
    ...
    def _calculate_q_vals(self, rewards: Sequence[np.ndarray]) -> Sequence[np.ndarray]:
        ...
        if not self.use_reward_to_go:
            ...
            q_values = [
                np.array(self._discounted_reward_to_go(reward)) for reward in rewards
            ]
    ...
    def _estimate_advantage(
        ...
        if self.normalize_advantages:
            ...
            advantages = (advantages - mean) / (std + 1e-8)  # avoid division by zero
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        T = len(rewards)
        discounted_factors = self.gamma ** np.arange(T)
        discounted_returns = np.full((T,), np.sum(discounted_factors * rewards))
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        for i in range(len(rewards) - 1, -1, -1):
            reward_to_go = rewards[i] + self.gamma * reward_to_go
